...
namespace VideosApi2.Controllers
{
    public class VideosController : ApiController
    {
        ...
        static VideosController()
        {
            ...
            InitialDirectory = HttpContext.Current.Server.MapPath("~/videos");
        }
        ...
        [HttpGet]
        [Route("api/videos/play")]
        public HttpResponseMessage Play(int videoId)
        {
            TARCLearnEntities db = new TARCLearnEntities();
            var video = db.Materials.FirstOrDefault(vid => vid.materialId == videoId);
            if(video == null)
            {
                return Request.CreateErrorResponse(HttpStatusCode.NotFound, "Video: " + videoId + " not found");
            }
            string f = video.materialName;
